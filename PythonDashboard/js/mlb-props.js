    ...
    async function renderMLBProps() {
      ...
      (function renderMLBDailyCards() {
        ...
        _renderRedditCard = function renderRedditCard() {
          ...
          const BASELINE = {
            // 2026-06-16 cutoff roll to 6/15: 6/11–6/15 baked into the AS-POSTED
            // record (graded through 6/15). Pinned here so the 2026-06-16 clean
            // backfill — which re-graded history under the 0.68 threshold — does
            // NOT disturb what was actually posted on Reddit.
            //   Posted record through 6/15:  picks 181-144 +4.50u
            //   Before codefix: 111-91 +0.08u  |  After codefix through 6/15: 70-53 +4.42u
            //   Weekly (current wk 6/15-, through cutoff 6/15): 1-2 -1.34u
            //   Yesterday 6/15: 1-2 -1.34u
            // Weekly = the CURRENT in-progress Mon-Sun week (resets each Monday);
            // this week's Monday is 6/15, so the baseline weekly covers 6/15.
            // `before_codefix` is fixed history (never changes). `codefix`
            // (= "after codefix") grows with each post-cutoff pick like `total`.
            cutoff: '2026-06-15',
            picks: {
              total:        { w: 181, l: 144, u:  4.50 },
              before_codefix:{ w: 111, l:  91, u:  0.08 },  // static, pre-codefix
              codefix:      { w:  70, l:  53, u:  4.42 },  // after codefix, through 6/15
              weekly:       { w:   1, l:   2, u:  -1.34 },  // current wk through cutoff (6/15)
              yesterday:    { w:   1, l:   2, u:  -1.34 },  // 6/15
            },
            leans: {
          ...
          (function renderMaeGate(){
            ...
            const band=(data.props||[]).filter(p =>
              (p.result==='WIN'||p.result==='LOSS') &&
              p.pCover!=null && p.pCover>=LOWT && p.pCover<HIGHT &&
              (p.pick==='OVER'||p.pick==='UNDER'||p.would_be_pick==='OVER'||p.would_be_pick==='UNDER'));
            const dts=[...new Set(g.map(p=>p.date))].sort();
            const dadd=(s,n)=>{const x=new Date(s+'T12:00:00');x.setDate(x.getDate()+n);return x.toISOString().slice(0,10);};
            function gap(D){
              const lo=dadd(D,-GWIN);
            ...
            for (const D of dts){
              const ga=gap(D);
              if (ga!=null && ga>=0){
                const dropped=band.filter(p=>p.date===D);
            ...
            if (tighten && typeof todayStr!=='undefined' && todayStr){
              const todMarg=(data.props||[]).filter(p=>p.date===todayStr &&
                (p.pick==='OVER'||p.pick==='UNDER'||p.would_be_pick==='OVER'||p.would_be_pick==='UNDER') &&
                p.pCover!=null && p.pCover>=LOWT && p.pCover<HIGHT).sort((a,b)=>b.pCover-a.pCover);
            ...
            if (flips.length){
              ...
              for (const f of flips.slice().reverse()){
                ...
                sum.textContent=`${f.date}  ·  3d gap +${f.gap.toFixed(2)}  ·  band ${f.w}-${f.l}  ·  saved ${((-f.u)>=0?'+':'')}${(-f.u).toFixed(2)}u`;
                det.appendChild(sum);
          ...
          for (const p of pageRows) {
            ...
            [displayName(p), p.team||'', p.opp||'',
             ...
             statusStr
              ...
              if (i===11 && p.pCover!=null) td.style.color = p.pCover >= MLB_PICK_STRONG ? 'var(--green)' : p.pCover >= MLB_LEAN_FLOOR ? 'var(--yellow)' : p.pCover >= MLB_WATCH_FLOOR ? 'var(--orange)' : p.pCover <= 0.45 ? 'var(--red)' : '#ccc';
          ...
          for (const p of pageRows) {
            ...
            [displayName(p), p.team||'', oppSP,
             ...
             isPick?(p.pick==='OVER'?'O':'U'):'\u2014'
              ...
              if (i===6 && p.pCover!=null) td.style.color = p.pCover >= MLB_PICK_STRONG ? 'var(--green)' : p.pCover >= MLB_LEAN_FLOOR ? 'var(--yellow)' : p.pCover >= MLB_WATCH_FLOOR ? 'var(--orange)' : p.pCover <= 0.45 ? 'var(--red)' : '#ccc';
