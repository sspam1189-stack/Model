...
import { fetchH2HMatchups } from "./sources/h2h_matchup.mjs";
...
const TOTAL_HALF_UNIT_START = "2026-03-07"; // totals picks bet at 0.5u from this date forward
const TOTAL_PICKS_END_DATE = "2026-03-09"; // stop taking totals picks from this date onward
...
// Returns unit P&L for a single total pick based on its date
function totalPickUnit(date, result) {
  const halfUnit = date >= TOTAL_HALF_UNIT_START;
  const win  = halfUnit ? 0.5  : 1.0;
  const loss = halfUnit ? -0.55 : UNIT_LOSS;
...
/* Change-detection logic - disabled (always send now)
// Extract actionable spread picks as a Set of "away@home:sPick" for comparison
function extractSpreadPicks(games) {
  const picks = new Set();
  for (const g of games || []) {
    if (g.status === "MISSING_ODDS" || g.status === "SKIPPED") continue;
    if (g.sPick && g.sPick !== "PASS" && isActionable(g.sConf)) {
      picks.add(`${g.away}@${g.home}:${g.sPick}`);
    }
  }
  return picks;
}

// Compare two sets of picks - returns true if they differ
function picksChanged(oldPicks, newPicks) {
  if (oldPicks.size !== newPicks.size) return true;
  for (const p of oldPicks) {
    if (!newPicks.has(p)) return true;
  }
  return false;
}
*/
...
// Known mascot names for fallback matching
...
function extractMascot(normalized) {
  const words = normalized.split(" ");
  const last = words[words.length - 1];
  if (MASCOTS.has(last)) return last;
  if (words.length >= 2) {
    const lastTwo = words.slice(-2).join(" ");
    if (MASCOTS.has(lastTwo)) return lastTwo;
  }
  return null;
}
...
function resolveKey(obj, teamName) {
  ...
  const aliased = TEAM_ALIASES[wanted] || wanted;

  for (const k of Object.keys(obj)) {
    const nk = normKey(k);
    if (nk === wanted || nk === aliased) return k;
    if (nk.startsWith(wanted) || nk.includes(wanted) || wanted.includes(nk)) return k;
    if (aliased !== wanted && (nk.includes(aliased) || aliased.includes(nk))) return k;
  }

  // Reverse alias: if an alias maps TO our input, check if its key matches an obj key
  // Handles: input="Oklahoma City Thunder", obj key="Okla City", alias "okla city"->"oklahoma city thunder"
  for (const [aliasKey, aliasVal] of Object.entries(TEAM_ALIASES)) {
    if (normKey(aliasVal) === wanted || normKey(aliasVal) === aliased) {
      for (const k of Object.keys(obj)) {
        if (normKey(k) === aliasKey) return k;
      }
    }
  }

  // Fallback: match on mascot name
  const wantedMascot = extractMascot(wanted) || extractMascot(aliased);
  if (wantedMascot) {
    for (const k of Object.keys(obj)) {
      const km = extractMascot(normKey(k));
      if (km && km === wantedMascot) return k;
    }
  }
...
// Same mascot matching for score grading
function matchTeam(a, b) {
  if (a === b) return true;
  const na = normKey(a), nb = normKey(b);
  if (na === nb) return true;
  if (na.includes(nb) || nb.includes(na)) return true;
  const ma = extractMascot(na), mb = extractMascot(nb);
...
function joinTrends(team, atsObj, ouObj) {
  ...
  if (!parts.length && (atsObj || ouObj)) {
    // Silently skip — resolved but no values is common
  }
  return parts.join(" · ");
}
...
async function gradeDateInStore(store, yyyymmdd) {
  ...
  for (const g of run.games || []) {
    ...
    if (!f) {
      // No score match — common for postponed/rescheduled games
      continue;
    }
    ...
    <div class="card card-records">
      ...
      <table class="data">
        <thead><tr>
          ...
          <th class="">Win%</th><th class="">Units</th>
...
function tallyPicks(list, { type = "spread", conf = null, side = null } = {}) {
  ...
  for (const g of list) {
    ...
    if (type === "total") totalUnits += totalPickUnit(g.date, result);
  }

  const pct = winPct(w, l);
  const units = type === "total" ? totalUnits : calcUnits(w, l);
...
function computeRollingWindows(store, type = "spread") {
  ...
  for (let i = 0; i + window <= picks.length; i += window) {
    ...
    const units = type === "total"
      ? slice.reduce((sum, x) => sum + totalPickUnit(x.date, x.result), 0)
      : calcUnits(w, l);
...
function computeWeeklyBreakdown(store, type = "spread") {
  ...
  for (const [week, entries] of [...byWeek.entries()].sort()) {
    ...
    const units = type === "total"
      ? entries.reduce((sum, x) => sum + totalPickUnit(x.date, x.result), 0)
      : calcUnits(w, l);
...
function tallyL10(rows, type = "spread") {
  ...
  const units = type === "total"
    ? rows.reduce((sum, x) => sum + totalPickUnit(x.date, x.result), 0)
    : calcUnits(w, l);
  ...
  const rows = (games || [])
    ...
    .map((g) => {
      ...
      return `<tr>
        ...
        <td>${sPickDisplay}${sConfBadge}<div class="tiny" style="margin-top:2px">Line ${fmtNum(g.line,1)} · proj ${margin}</div></td>
        <td style="text-align:center">${pCoverStr}<div class="tiny">${pAway} away / ${pHome} home</div></td>
        <td>${oPickDisplay}${oConfBadge}<div class="tiny" style="margin-top:2px">O/U ${fmtNum(g.total,1)} · diff ${tDiff}</div></td>
        <td style="text-align:center">${pOUStr}<div class="tiny">${pOver} over / ${pUnder} under</div></td>
      </tr>`;
    }).join("");
  ...
  return `<div class="card" style="border-left:4px solid #8b5cf6; margin-bottom:10px;">
    ...
    <table class="data">
      <thead><tr>
        ...
        <th>Total Pick</th>
        <th style="text-align:center">P(Hit)</th>
      </tr></thead>
      <tbody>${rows}</tbody>
    </table>
...
function buildEmailHtml(run, summaryObj, last10, last10Totals, weeklySpread, weeklyTotal, rollingSpread, rollingTotal, teamRecords, calibRows, yesterdayRecap) {
  ...
  const buildPicksList = (games, type) => {
    ...
    // Skip totals output if past the cutoff date
    if (!isSpread && run.date >= TOTAL_PICKS_END_DATE) {
      ...
      .map((g) => {
        if (isSpread) {
          ...
          return `<div style="padding:6px 0; border-bottom:1px dashed #eef2f7;">🏀 <span class="pick-team">${esc(g.sPick)}</span>` +
            ` <span class="trend-label">proj ${esc(favTeam)} by ${fmtNum(Math.abs(projMargin), 1)} · edge ${fmtNum(g.sDiff, 1)}${pStr}</span>` +
  ...
  const buildWeeklyCard = (title, rows) =>
    ...
    buildTable(rows, [
      ...
      { label: "Units", center: true, render: (r) => unitsHtml(r.units) },
    ]) + `</div>`;

  const buildRollingCard = (title, rows) => {
    // Detect window size from first row label (e.g. "#1–10" or "#1–20")
  ...
  const gameCards = (run.games || []).map((g) => {
    ...
    const spreadPick = !isSkipped && g.sPick && g.sPick !== "PASS"
      ? (() => {
          const projMargin = Math.round((g.hS - g.aS) * 10) / 10;
          const favTeam = projMargin >= 0 ? g.home : g.away;
          return `<div><span class="pick-team">${esc(g.sPick)}</span> ${confBadge(g.sConf)} <span class="trend-label">proj ${esc(favTeam)} by ${fmtNum(Math.abs(projMargin), 1)} · edge ${fmtNum(g.sDiff, 1)}${g.pCover ? ` · P=${(g.pCover*100).toFixed(0)}%` : ""}</span></div>`;
        })()
  ...
  return `<html><head>${EMAIL_STYLES}</head><body>
    <div class="wrap">
      <table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-bottom:10px;">
        <tr>
          <td>
            <div class="h1">NBA Picks (Full Season) — ${esc(run.dateDisplay || "")}</div>
      ...
      ${/* Total Trends - commented out
      ${sectionLabel("Total Trends")}
      ${rowFull(buildLast10Card("🧾 Last 10 Total", last10Totals || [], "total"))}
      ${rowFull(buildWeeklyCard("📅 Weekly Total", weeklyTotal || []))}
      ${rowFull(buildRollingCard("📊 Rolling {n}-Pick Groups (Total)", rollingTotal || []))}
      */ ""}
...
function buildTextEmail(run, store) {
  const lines = [`NBA PICKS — ${run.dateDisplay}`, "", "TODAY (Actionable only)", ""];
  ...
  for (const g of run.games || []) {
    ...
    if (g.injuryNote) lines.push(`  Injury: ${g.injuryNote}`);
    if (g.trends) {
...
async function main() {
  ...
  console.log(`\n══ NBA Picks (Full Season) Pipeline — ${dateDisplay} ══\n`);
  ...
  const [enhancedStats, odds, ats, ou, injuryData, playerAdvanced, b2bTeams, h2hMatchups] = await Promise.all([
    ...
    fetchInjuryData(null, { seasonType, espnType }).catch((e) => {
      console.warn("  ⚠ Injury fetch failed:", e.message);
      return { report: {}, playerMPG: {} };
    }),
    fetchPlayerAdvanced({ seasonType }).catch((e) => {
      console.warn("  ⚠ Player advanced fetch failed:", e.message);
      return {};
    }),
  ...
  // Cache per-team deltas (adjusted - blended) so recalculate can replay them.
  // 6 stat keys × 2 teams per game. Stored on each game record.
  const STAT_KEYS = ["OFF", "DEF", "TS", "TO", "ORR", "PACE"];
  function computeTeamDelta(team) {
    const base = stats[team];
  ...
  for (const g of odds) {
    ...
    const r = analyzeGame(g, gameStats, gameAvgs, baseW, injuryAdj, kalmanState, baseWVar, dynamicResidualVar, h2hMatchups);
    ...
    // Cache lineup/B2B deltas for recalculate replay
  ...
  // 6. Build run record (weights already tuned in step 3b)
  ...
  const subject = `[JS] NBA Full Season Update Picks \u2014 ${run.dateDisplay}`;
