...
export function projScore(team, opp, isHome, H, a, W, kalmanAdj = null, W_var = null, residualVar = null) {
...
function computeH2HAdj(homeTeam, awayTeam, h2hMatchups, W) {
  ...
  const h2hWeight = W.h2hWeight ?? 0.15;
  const maxAdj = 4.0;
...
export function analyzeGame(g, H, a, W, injuryAdj = null, kalmanState = null, W_var = null, residualVar = null, h2hMatchups = null) {
  ...
  // Project scores
  const aProj = projScore(gg.away, gg.home, false, H, a, W, awayKalman, W_var, residualVar);
  const hProj = projScore(gg.home, gg.away, true, H, a, W, homeKalman, W_var, residualVar);
  ...
  // ── H2H matchup adjustment ────────────────────────────────────────────
  // Shifts margin based on how these specific teams perform against each
  // other. Same pattern as Kalman — correction baked into the number.
  const h2h = computeH2HAdj(homeKey, awayKey, h2hMatchups, W);
  ...
  if (useBayesian) {
    ...
    if (bestSpreadP >= probH && sDiff <= 10 && absLine < 12) {
      if (spreadSide === "home") {
    ...
    if (sDiff >= W.sprHigh && sDiff <= 10 && absLine < 12) {
      sPick = margin > 0
