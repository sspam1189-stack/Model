...
function buildGameProbTable(games) {
  const rows = (games || [])
    ...
    .map((g) => {
      ...
      // colour the pick's directional probs
...
function buildEmailHtml(run, summaryObj, last10, last10Totals, weeklySpread, weeklyTotal, rollingSpread, rollingTotal, teamRecords, calibRows, yesterdayRecap) {
  ...
  return `<html><head>${EMAIL_STYLES}</head><body>
    <div class="wrap">
      ...
      ${row2col(
        `<div class="card card-picks"><div class="summaryTitle">🔥 Today's Spread Picks (Actionable)</div>${buildPicksList(run.games || [], "spread")}</div>`,
        buildCombinedRecordTable("📊 Spread (ATS)", s.spread.elite, [
...
async function main() {
  ...
  const subject = `[JS] NBA Full Season Recap Picks \u2014 ${run.dateDisplay}`;
