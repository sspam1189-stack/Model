...
# 2026-07-06: generalized from the threes-only fallback to every tab. Since
# 7/2 the strict map has matched nothing but threes on WNBA event pages —
# threes alone survived because it alone had a keyword fallback — so a
# FanDuel marketType rename silently blanked points/rebounds/assists from
# the model. Each entry: (internal_market, required_keywords_any,
# excluded_keywords). A marketType matches if ANY required keyword is
# present and NO excluded keyword is (guards against alt lines, team/game
# markets, and combo markets bleeding into single-stat tabs).
_TAB_FALLBACK = {
