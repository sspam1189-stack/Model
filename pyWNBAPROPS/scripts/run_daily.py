#!/usr/bin/env python3
"""
pyWNBAPROPS/scripts/run_daily.py
Daily WNBA player prop projection pipeline with Kalman filtering.

Stages:
  1. Load Kalman state (or initialize if first run)
  2. Fetch player game logs from stats.wnba.com (via nba_api league_id='10')
  3. Update Kalman state with any new games
  4. Fetch team defensive stats for opponent adjustment
  5. Fetch today's prop lines from FanDuel / The Odds API (basketball_wnba)
...
Usage:
    ...
    python -m scripts.run_daily --date 20250516
"""
...
from sources.wnba_player_stats import (
...
from sources.odds_fanduel import fetch_fanduel_wnba_props
from sources.odds_theoddsapi import fetch_wnba_player_props
...
def _get_todays_teams(date_key):
    ...
    """
    espn_path = os.path.join(SCRIPT_DIR, "..", "data", "espn_cache", "wnba", f"{date_key}.json")
    ...
    # ESPN WNBA abbreviations -> nba_api/FanDuel stats abbreviations.
    espn_to_fd = {"GS": "GSV", "LA": "LAS", "LV": "LVA", "NY": "NYL",
                  "WSH": "WAS", "POR": "PDX"}
...
def run_daily(date_key=None):
    ...
    print(f"\n{'='*60}")
    print(f"  WNBA PLAYER PROPS (Kalman) — {date_iso}")
    ...
    prop_lines = fetch_fanduel_wnba_props(date_key=date_key)
    if not prop_lines:
        print(f"  FanDuel returned 0 lines, falling back to The Odds API...")
        prop_lines = fetch_wnba_player_props(date_key=date_key)
    else:
        # 2026-07-06: FanDuel's event pages can carry only part of the board
        # (since 7/2 the morning fetch has returned threes only). Backfill any
        # core market FanDuel didn't yield from The Odds API — whose response
        # includes FanDuel's own book, now preferred per-market — instead of
    ...
    if injury_report:
        ...
        print(f"  {n_out} players out/doubtful across {len(injury_report)} teams")
    else:
    ...
    try:
        ...
        _tom_fanduel = fetch_fanduel_wnba_props(date_key=_tom_key) if _tom_teams else []
