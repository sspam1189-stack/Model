...
def backfill(season=None, start_game=15, start_date=None, use_real_lines=True):
    """
    ...
    print(f"\n{'='*60}")
    print(f"  WNBA PLAYER PROPS BACKTEST (Bayesian Kalman)")
    ...
    for date_idx, game_date in enumerate(all_dates):
        ...
        # Phase 3: Load REAL closing prop lines from The Odds API historical
        # endpoint (odds_theoddsapi_historical). Reads the per-date parsed
        # cache first; only bills the API if a date isn't cached yet (requires
        # ODDS_HIST_KEY / ODDS_API_KEY in env). WNBA has no free live-historical
        # path, so this is the only real-line source.
        real_lines = None
        try:
            from sources.odds_theoddsapi import _props_cache_path, _load_cache
            from sources.odds_theoddsapi_historical import backfill_date
            date_key = game_date.replace("-", "")
            cp = _props_cache_path(date_key)
            cached = _load_cache(cp, max_age_hours=None)
            # 2026-07-06: a cache written by the LIVE FanDuel fetch can be a
            # partial board (since 7/2 those carried only threes). The
            ...
            if cached is not None and not _partial_live:
                ...
                if _partial_live:
                    ...
                    print(f"  [{game_date}] No cached props — fetching real closing lines...")
                real_lines, credits, _ = backfill_date(game_date, force=_partial_live)
                if not real_lines:
                    if _partial_live:
                        real_lines = cached
                        ...
                        print(f"  [{game_date}] No historical props available — skipping date")
                elif credits:
                    print(f"  [{game_date}] Fetched {len(real_lines)} real lines ({credits} credits)")
        except Exception as e:
...
def _print_summary(results, total_projected, season):
    ...
    print(f"\n{'='*60}")
    print(f"  WNBA PROPS BACKTEST SUMMARY (Kalman) — {season}")
    print(f"{'='*60}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--season", type=str, default=None,
                        help="WNBA season string (e.g. 2025) (e.g. '2025-26')")
