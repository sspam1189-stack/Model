# pyWNBAPROPS/scripts/player_kalman.py
# Per-player Kalman filter for WNBA player prop projections.
#
# Fork of pyNBAPROPS's player_kalman.py. Hyperparameters retuned for the WNBA:
#   - obsNoise set to measured WNBA within-player game-to-game variance
#     (2024+2025, players with >=15 qualifying games): pts 27, reb 5, ast 2.5,
#     fg3m 1.0. The NBA defaults over-stated ast/fg3m noise (5.0 / 2.5), which
#     made the filter too sluggish on those low-count WNBA stats.
#   - minGamesForKalman lowered 5 -> 4 to match the short-season MIN_GAMES.
#   - initialVar / gameDrift widened so early-season WNBA players (short ~40
#     game season) adapt quickly instead of being over-smoothed toward a
#     stale prior.
#
...
PLAYER_KALMAN_DEFAULTS = {
    ...
    # Widened 0.3 -> 0.4 for the short WNBA season so the filter re-adapts
    # faster after absences (fewer games means each carries more signal).
    "gameDrift": 0.4,          # Variance increase per game missed

    # Observation noise per stat = measured WNBA within-player game-to-game
    # variance (2024+2025, players with >=15 qualifying games).
    "obsNoise": {
        "pts":   27.0,         # WNBA within-player std ~5.2 -> var ~27
        "reb":    5.0,         # std ~2.2 -> var ~5
        "ast":    2.5,         # std ~1.6 -> var ~2.5 (NBA used 5.0; too sluggish)
        "fg3m":   1.0,         # std ~1.0 -> var ~1.0 (NBA used 2.5)
        "stl":    0.6,
        "blk":    0.6,
        "tov":    1.6,
    },

    # Initial variance for a new player (high uncertainty). Bumped 20 -> 24 so
    # early-season WNBA players (short season) trust incoming games more.
    "initialVar": 24.0,
    ...
    # Lowered 5 -> 4 to match the short-season MIN_GAMES gate.
    "minGamesForKalman": 4,
