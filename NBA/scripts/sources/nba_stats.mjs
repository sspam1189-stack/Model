...
const PLAYOFF_RAMP_GAMES = 16;  // full playoff weight after this many games
...
function blendPlayoffStats(regSeason, playoffStats) {
  ...
  for (const [team, rs] of Object.entries(regSeason)) {
    ...
    if (!po || !po.GP || po.GP < 1) {
      // Team not in playoffs or no games yet — use regular season
      blended[team] = { ...rs };
      continue;
    }

    // Ramp: more playoff games → more playoff weight
...
export async function fetchNBAStatsEnhanced(dateTo = null, { seasonType = "Regular Season" } = {}) {
  ...
  try {
    // Home/away splits: use regular season in playoffs (too few playoff home/away games)
    const splitType = inPlayoffs ? "Regular Season" : seasonType;
    home = await fetchTeamStats(dateTo, null, { location: "Home", seasonType: splitType });
    console.log(`  [nba_stats] Home splits: ${Object.keys(home).length} teams`);
    await sleep(2000);
  } catch (e) {
    console.warn(`  [nba_stats] Home fetch failed (${e.message}) — skipping`);
    home = null;
  }

  try {
    const splitType = inPlayoffs ? "Regular Season" : seasonType;
    away = await fetchTeamStats(dateTo, null, { location: "Road", seasonType: splitType });
    console.log(`  [nba_stats] Away splits: ${Object.keys(away).length} teams`);
  } catch (e) {
    console.warn(`  [nba_stats] Away fetch failed (${e.message}) — skipping`);
    away = null;
  }

  // Filter season stats for min games (backward compat)
  for (const [k, v] of Object.entries(season)) {
    if (v.GP < 10) delete season[k];
  }
