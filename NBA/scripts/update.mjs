  ...
  for (const g of yesterdayRun.games || []) {
    ...
    if (g.sPick && g.sPick !== "PASS" && isActionable(g.sConf)) {
      ...
      if (result) {
        ...
        // Calculate units (flat 1u: WIN = +1, LOSS = -1.1)
        if (result === "WIN") {
          units += 1;
        } else if (result === "LOSS") {
          units -= 1.1;
  ...
  return `<html><head>${EMAIL_STYLES}</head><body>
    <div class="wrap">
      <table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-bottom:10px;">
        <tr>
          <td>
            <div class="h1">NBA Picks — ${esc(run.dateDisplay || "")}</div>
...
async function main() {
  ...
  console.log(`\n══ NBA Picks Pipeline — ${dateDisplay} ══\n`);
  ...
  const subject = `[JS] NBA Update Picks \u2014 ${run.dateDisplay}`;
