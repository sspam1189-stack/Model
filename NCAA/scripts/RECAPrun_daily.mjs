// scripts/RECAPrun_daily.mjs
// Daily NCAA picks pipeline (with Yesterday's Recap):
//   1. Grade yesterday's picks against final scores
//   2. Compute yesterday's recap (flat 1u betting, no Kelly)
//   3. Fetch today's stats, odds, trends
//   4. Analyze each game via model engine
//   5. Build HTML + text email with recap, records, trends, rolling windows
//   6. Send via email + Discord (recap as first Discord message)
...
function buildDiscordMessages(run, store, yesterdayDate) {
  ...
  // ── Message 1: Yesterday's Recap ──
  const recapLines = [];
  recapLines.push(`**📋 NCAAB Recap — ${toDisplayDate(yesterdayDate)}**`);
  const yesterdayRun = (store.runs || []).find((r) => r.date === yesterdayDate);
  if (yesterdayRun) {
    ...
    if (recapPicks.length) {
      ...
      for (const g of recapPicks) {
        ...
        recapLines.push(`> **${g.away} @ ${g.home}** — ${emoji} ${g.sPick}`);
      }
      recapLines.push(`> **${sW}W-${sL}L-${sP}P ${fmtUnits(sUnits)}**`);
    } else {
      recapLines.push(`> No actionable picks graded.`);
    }
  } else {
    recapLines.push(`> No run found.`);
  }

  messages.push(recapLines.join("\n"));

  // ── Message 2: Today's Final Picks ──
  const lines = [];
  lines.push(`**🔥 NCAAB FINAL PICKS — ${run.dateDisplay}**`);
  lines.push("");
  ...
  const totalPicks  = todayGames.filter((g) => g.oPick && g.oPick !== "PASS" && isActionable(g.oConf));
  ...
  // ── Message 3: Season Record + Last 10 ──
  const s = computeSummaryObj(store);
  const recordLines = [];
  recordLines.push("**📊 NCAAB SEASON RECORD**");
  recordLines.push("");
