# pyMLB/scripts/self_tune.py
# Standalone MLB Bayesian weight tuner.
#
# This is a full reimplementation for MLB weight keys. We do NOT import
...
# MLB-specific config pulled from defaults
...
# MLB weight keys for the Bayesian margin update (Signal 1)
# These map to _marginFeatures produced by model_engine._extract_margin_features.
# From defaults.py:
_WEIGHT_KEYS = defaults._WEIGHT_KEYS   # 18 MLB weight keys
_FEATURE_KEYS = defaults._FEATURE_KEYS  # 18 MLB feature delta keys

# MLB weight clamp range (all non-hfa weights)
_MLB_CLAMP_LO = 0
_MLB_CLAMP_HI = 10
...
def tune_weights(current_w, current_w_var, completed_rows):
    """
    MLB-specific Bayesian weight tuner.
    ...
    Four signals:
      ...
      4. Adaptive Kalman parameters (gameNoise, dailyDrift)
    ...
    # ==================================================================
    # SIGNAL 1: Bayesian weight update on MLB margin features
    # ==================================================================
    ...
    # Clamp MLB weights: non-hfa weights 0-10, hfa within HCA bounds
    for k in _WEIGHT_KEYS:
        if k == "hfa":
            W[k] = _clamp(W[k], HCA_CLAMP_LO, HCA_CLAMP_HI)
        else:
            W[k] = _clamp(W[k], _MLB_CLAMP_LO, _MLB_CLAMP_HI)

    if n_bayes > 0:
        print(f"  [self_tune] Bayesian update on {n_bayes} games ->"
              f" wStarterFIP={W['wStarterFIP']} (v={W_var['wStarterFIP']:.3f})"
              f" wStarterKBB={W['wStarterKBB']} (v={W_var['wStarterKBB']:.3f})"
              f" hfa={W['hfa']} (v={W_var['hfa']:.3f})"
              f" wWRCPlus={W['wWRCPlus']} (v={W_var['wWRCPlus']:.3f})")
    ...
    # MLB has fixed 9 innings — no paceAdj, gradient descent on constant only.
    # ==================================================================
    ...
    for r in completed_rows:
        if (not _is_finite(r.get("homeScore")) or not _is_finite(r.get("awayScore"))
                or not _is_finite(r.get("hS")) or not _is_finite(r.get("aS"))):
            continue
        actual_margin = r["homeScore"] - r["awayScore"]
        pred_margin = r["hS"] - r["aS"]
