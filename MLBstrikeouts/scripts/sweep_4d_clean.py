#!/usr/bin/env python3
"""
4D sweep: BF_MULT x BF_CAP x VAR_MULT x K_RATE_CAP_FLOOR.
Clean emp_std per combo.

Grid (36 combos, ~20-25 min):
  BF:   [1.20, 1.30]
  CAP:  [23, 25, 26]
  VAR:  [1.30, 1.40]
  KCAP: [0.36, 0.38, 0.40]
...
Usage:
    cd MLBstrikeouts
    python -m scripts.sweep_4d_clean
"""
...
def main():
    ...
    cap_grid = [23, 25, 26]
    var_grid = [1.30, 1.40]
    kcap_grid = [0.36, 0.38, 0.40]
    total = len(bf_grid) * len(cap_grid) * len(var_grid) * len(kcap_grid)

    b_bf = defaults.BF_MULT
    b_var = dict(defaults.VAR_MULT)
    b_cap = defaults.BF_CAP
    b_kcap = defaults.K_RATE_CAP_FLOOR
            ...
            for cap in cap_grid:
                for v in var_grid:
                    for kc in kcap_grid:
                        ...
                        defaults.K_RATE_CAP_FLOOR = kc
                        _wipe_emp_std()
                        buf = io.StringIO()
                        with contextlib.redirect_stdout(buf):
                            r = backfill()
                        picks = (r or {}).get('strikeouts', {}).get('picks', []) or []
                        results[(bf, cap, v, kc)] = picks
                        np_ = sum(1 for p in picks if is_pick(p))
                        nl = sum(1 for p in picks if is_lean(p))
                        print(f'  [{i:2d}/{total}] BF={bf:.2f} CAP={cap} VAR={v:.2f} KCAP={kc:.2f}  picks={np_} leans={nl}')
                        sys.stdout.flush()
    finally:
    ...
    for key, pp in results.items():
        ...
        rows.append((bf, cap, v, kc, s, sr))

    print()
    print('TOP 15 by RECENT PICKS-ONLY ROI:')
    print(f'  {"BF":>4} {"CAP":>3} {"VAR":>4} {"KCAP":>4}  {"recent picks":>26}  {"season picks":>26}')
    rows.sort(key=lambda x: -x[5]['p_roi'])
    for bf, cap, v, kc, s, sr in rows[:15]:
        print(f'  {bf:>4.2f} {cap:>3d} {v:>4.2f} {kc:>4.2f}  '
