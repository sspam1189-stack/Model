...
FADE_WINDOW = {
    ...
    "Skenes":         [{"add": SEASON_START, "remove": "2026-07-31"},
                       # 2026-08-26: briefly closed after the 8/25 fade lost
                       # to a 1-0 gem, then reopened the same day (user) --
                       # one start is not enough to kill the 8/20 re-add
                       # thesis. Watch: if his walks stay down over the next
                       # 2-3 starts, the form case is gone and this closes
                       # for real.
                       {"add": "2026-08-20"}],
    ...
    # Removed AGAIN 2026-08-31 (user): the rookie adjusted (Painter/McLean
    # pattern) -- 2.37 ERA / 1.02 WHIP / 32.8 K% over his last 5, the fade
    # lost 7 of his last 8 starts, and his home 6-3 vs the BOS-home blind
    # fade baseline (+5.6%/play) is p~0.2 on 9 starts. Same-day removal
    # dropped the 8/31 SEA play.
    # RE-ADDED 2026-09-01 (user), the third flip-flop on this arm (removed
    # 8/19, reverted same day; removed 8/31, re-added next day). Home-only
    # unchanged (see FADE_VENUE, home fade-every-start 7-4 +4.24u but 1-3
    # since 7/20). 8/31 stays a no-play gap. The removal case above was not
    # rebutted by new evidence -- this is a user conviction call; if the
    # next 2-3 home fades lose, the removal reasoning stands confirmed.
    "Tolle": [
        {"add": SEASON_START, "remove": "2026-08-31"},
        {"add": "2026-09-01"},
    ],
