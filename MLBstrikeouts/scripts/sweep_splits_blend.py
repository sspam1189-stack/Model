#!/usr/bin/env python3
"""
sweep_splits_blend.py — grid-search SPLITS_BLEND_WEIGHT.

Same harness as sweep_season_anchor.py; varies SPLITS_BLEND_WEIGHT which
controls how much rolling-45d split (vs LHB / vs RHB) is mixed with the
season-to-date split.  0.0 = pure season split, 1.0 = pure recent split.

Usage:
    cd MLBstrikeouts
    python -m scripts.sweep_splits_blend
"""
...
def _run_at(weight):
    defaults.SPLITS_BLEND_WEIGHT = weight
    buf = io.StringIO()
...
def main():
    ...
    print(f"\n  Sweeping SPLITS_BLEND_WEIGHT over {weights}\n")
