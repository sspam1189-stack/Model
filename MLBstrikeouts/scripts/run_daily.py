        ...
        try:
            ...
            for date_entry in sched.get("dates", []):
                for game in date_entry.get("games", []):
                    ...
                    # Only treat lineups as confirmed once the game is actually
                    # underway. MLB statsapi populates lineups.homePlayers /
                    # awayPlayers with projected/prior-game lineups well before
                    # games start, so length alone falsely flags teams as
                    # confirmed in early-morning runs and the merge logic drops
                    # fresh picks as "first-time entry on a locked game".
                    game_status = (game.get("status") or {}).get("abstractGameState", "")
                    game_started = game_status in ("Live", "Final")
                    for side, lineup_key in [("home", "homePlayers"), ("away", "awayPlayers")]:
                        ...
                        confirmed = (
                            len(lineup_entries) >= 9 and game_started
                        )
