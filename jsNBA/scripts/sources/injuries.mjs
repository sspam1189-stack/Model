...
async function fetchMPG_playerindex(seasonType = "Regular Season") {
  ...
  const params = new URLSearchParams({
    ...
    SeasonType: seasonType, TeamID: "0", Weight: "",
  });
...
async function fetchMPG_espn(espnType = 2) {
  ...
  // espnType: 2 = regular season, 3 = playoffs
  const urls = [
    ...
    `https://sports.core.api.espn.com/v2/sports/basketball/leagues/nba/seasons/2026/types/${espnType}/leaders?limit=500`,
...
export async function fetchPlayerMPG({ seasonType = "Regular Season", espnType = 2 } = {}) {
  // Always use Regular Season for MPG — playoff sample is too small for reliable
  // minutes data, and player importance tiers shouldn't change between RS and playoffs.
  const mpgSeasonType = "Regular Season";
  const mpgEspnType = 2;
  ...
  try {
    const data = await fetchMPG_leaguedash(mpgSeasonType);
  ...
  try {
    mpgMap = await fetchMPG_espn(mpgEspnType);
