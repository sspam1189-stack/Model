...
async function fetchTeamOnOff(teamName) {
  ...
  for (const row of onSet.rowSet) {
    ...
    if (!name) continue;
    result[name] = { onOFF: row[hi(onSet.headers, "OFF_RATING")], onDEF: row[hi(onSet.headers, "DEF_RATING")], onMin: row[hi(onSet.headers, "MIN")], offOFF: null, offDEF: null, offMin: 0 };
  }
  for (const row of offSet.rowSet) {
    const name = row[hi(offSet.headers, "VS_PLAYER_NAME")];
    if (!name || !result[name]) continue;
    result[name].offOFF = row[hi(offSet.headers, "OFF_RATING")];
    result[name].offDEF = row[hi(offSet.headers, "DEF_RATING")];
    result[name].offMin = row[hi(offSet.headers, "MIN")];
  }
...
function computeOnOffAdj(outPlayerNames, onOffData, teamOFF, teamDEF) {
  ...
  for (const outName of outPlayerNames) {
    ...
    const onOffKey = Object.keys(players).find(k => {
      const parts = k.split(", ");
      return (parts[0]?.toLowerCase() === lastName && parts[1]?.toLowerCase() === firstName.toLowerCase()) || k === outName;
    });
    if (onOffKey && players[onOffKey].offOFF != null && players[onOffKey].onMin >= 200) {
      const p = players[onOffKey];
      matched.push({ name: outName, onOFF: p.onOFF, offOFF: p.offOFF, onDEF: p.onDEF, offDEF: p.offDEF, onMin: p.onMin, offDelta: p.onOFF - p.offOFF, defDelta: p.onDEF - p.offDEF });
    }
  }
  ...
  for (let i = 1; i < matched.length; i++) {
    ...
    adjOFF -= p.offDelta * onFrac;
    adjDEF -= p.defDelta * onFrac;
    parts.push(`${p.name}: delta=${p.offDelta.toFixed(1)} × ${(onFrac*100).toFixed(0)}% = ${(p.offDelta*onFrac).toFixed(2)}`);
  }
  console.log(`  [lineup/onoff] ${parts.join(" | ")}`);
  console.log(`  [lineup/onoff] Adjusted OFF: ${adjOFF.toFixed(1)} (delta ${(adjOFF-teamOFF)>=0?"+":""}${(adjOFF-teamOFF).toFixed(1)}), DEF: ${adjDEF.toFixed(1)} (delta ${(adjDEF-teamDEF)>=0?"+":""}${(adjDEF-teamDEF).toFixed(1)})`);
  return { offAdj: adjOFF - teamOFF, defAdj: adjDEF - teamDEF };
}
    ...
    if (onOffResult) {
      ...
      const weightedAvg = (players, getter, weightFn) => {
        ...
        for (const p of players) {
          const val = getter(p);
          const w = weightFn(p);
          if (Number.isFinite(val) && Number.isFinite(w) && w > 0) { sum += w * val; totalW += w; }
        }
      ...
      if (fullOFF != null && availOFF != null) { adj.OFF = Math.round((orig.OFF + (availOFF - fullOFF)) * 100) / 100; anyChange = true; }
      if (fullDEF != null && availDEF != null) { adj.DEF = Math.round((orig.DEF + (availDEF - fullDEF)) * 100) / 100; anyChange = true; }
    }
