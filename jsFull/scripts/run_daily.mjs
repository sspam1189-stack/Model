...
async function main() {
  ...
  if (date <= "20260420" && enhancedStats.last10?.["Orlando Magic"]) {
    try {
      const { default: NBAApi } = await import("nba-api-client").catch(() => ({ default: null }));
      // Use dynamic import for nba_api equivalent, or fetch directly
      ...
      if (_glRes?.ok) {
        ...
        const _iPTS = _glHeaders.indexOf("PTS");
        const _iMU = _glHeaders.indexOf("MATCHUP");

        // Exclude the two blowout games by Game_ID
        ...
        if (_l10Games.length === 10) {
          // Fetch advanced box scores for each game
          const _advStats = [];
  ...
  try {
    ...
    let _injToWrite = { injuryData, playerAdvanced, h2hMatchups };
    try {
  ...
  const filteredOdds = odds.filter(g => {
    if (!g.startTimeUTC) return true;  // no time info, keep it
    try {
    ...
    if (recentInjuryDates && Object.keys(recentInjuryDates).length >= 5) {
      const detectReturns = (teamName) => {
        ...
        for (const [pName, pInfo] of Object.entries(mpgData)) {
          ...
          if (pInfo.mpg < 22) continue;
          if (todaysOutNames.has(pName)) continue;
  ...
  const subject = `[JS] NBA Full Season Daily Picks \u2014 ${run.dateDisplay}`;
