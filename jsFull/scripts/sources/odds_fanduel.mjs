...
const FD_API_KEY = "FhMFpcPWXMeyZxOx"; // Public key embedded in FD website
...
export async function fetchFanDuelNBAOdds() {
  const url = `${FD_BASE}/content-managed-page?page=CUSTOM&customPageId=nba&_ak=${FD_API_KEY}`;
  let data;
  try {
      ...
      if (mt.includes("HANDICAP") && mt.includes("2-WAY")) {
        for (const r of m.runners || []) {
          ...
          if (h != null && Number(h) < 0) {
            line = toModelLine(home, away, Number(h), r.runnerName || "");
          }
        }
      }
  ...
  console.log(`  [fanduel] Fetched ${games.length} NBA games with spreads/totals`);
  return games;
}
