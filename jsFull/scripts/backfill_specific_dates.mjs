...
async function main() {
  ...
  for (const date of targetDates) {
    ...
    // jsFull: B2B set built from ALL scored games (includes SKIPPED), not just graded
    const b2bTeams = new Set();
    for (const g of (prevRun?.games || [])) {
      if (g.status === "MISSING_ODDS") continue;
    ...
    // Per-team adj deltas for recalculate replay (jsFull pattern)
