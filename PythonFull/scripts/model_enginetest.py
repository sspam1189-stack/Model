...
def proj_score(team, opp, is_home, H, a, W, kalman_adj=None, W_var=None):
    ...
    variance = BAYES_HYPER["residualVar"]  # baseline irreducible noise
...
def build_injury_note(injury_adj):
    if not injury_adj:
        return None
    parts = []
    away_inj = injury_adj.get("awayInjuries") or []
    home_inj = injury_adj.get("homeInjuries") or []
    if away_inj:
        parts.append("Away: " + ", ".join(f"{i['player']} ({i['status']}/{i['tier']})" for i in away_inj))
    if home_inj:
        parts.append("Home: " + ", ".join(f"{i['player']} ({i['status']}/{i['tier']})" for i in home_inj))
    return " | ".join(parts) if parts else None


# ── Game Analysis ───────────────────────────────────────────────────────────

def analyze_game(g, H, a, W, injury_adj=None, kalman_state=None, W_var=None):
