...
def tune_weights(current_w, current_w_var, completed_rows):
    ...
    for r in completed_rows:
        ...
        recency_w = r.get("_recencyWeight")
        recency_w = recency_w if _is_finite(recency_w) else 1.0
    ...
    W["hca"]  = _clamp(W["hca"],  1.5, 3.5)  # NBA HCA is well-studied: 2-3 pts.
    ...
    for r in completed_rows:
        ...
        w = r.get("_recencyWeight")
        w = w if _is_finite(w) else 1.0
...
def _is_finite(x):
    """Check if a value is a finite number (equivalent to Number.isFinite in JS)."""
    if x is None:
        return False
    try:
