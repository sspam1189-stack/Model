...
def get_avgs(H):
    ...
    n = len(teams) or 1
    return {
        "ts": sum(x.get("TS", 0) for x in teams) / n,
        "to": sum(x.get("TO", 0) for x in teams) / n,
        "orr": sum(x.get("ORR", 0) for x in teams) / n,
    }
...
def resolve_team(H, name):
    ...
    for k in keys:
        if norm_key(k) == wanted:
            return k
    expanded = norm_key(expand_team_name(name))
    for k in keys:
        if norm_key(k) == expanded:
            return k
    for k in keys:
        nk = norm_key(k)
        if nk in wanted or wanted in nk or nk in expanded or expanded in nk:
            return k
    return None


# Projection: Returns { score, variance }
def proj_score(team, opp, is_home, H, a, W, kalman_adj=None, w_var=None):
    ...
    variance = BAYES_HYPER.get("residualVar", 130)
    ...
    if w_var:
        ...
        weight_var = (
            (d_ts * pace) ** 2 * (w_var.get("wTS", 0) or 0) +
            (d_to * pace) ** 2 * (w_var.get("wTO", 0) or 0) +
            (d_orr * pace) ** 2 * (w_var.get("wORR", 0) or 0) +
            (0.5 * d_net * pace) ** 2 * (w_var.get("wNET", 0) or 0) +
            (1 if is_home else 0) * (w_var.get("hca", 0) or 0)
        )
        variance += weight_var
...
def proj_total(home_team, away_team, H, a, W):
    ...
    pace = (((h["PACE"] + aw["PACE"]) / 2) * (W.get("paceAdj", 1) or 1)) / 100 * PACE_SCORING_FACTOR

    return round(total_base * pace * 10) / 10


# Feature vector for self_tune margin regression
...
# Injury note builder
def build_injury_note(injury_adj):
    if not injury_adj:
        return None
    parts = []
...
# Game Analysis
def analyze_game(g, H, a, W, injury_adj=None, kalman_state=None, w_var=None, residual_var=None):
    away_key = resolve_team(H, g.get("away"))
    home_key = resolve_team(H, g.get("home"))
    ...
    if kalman_state and kalman_state.get("teams"):
        teams = kalman_state["teams"]
        ht = teams.get(home_key)
        at = teams.get(away_key)
        if ht:
            home_kalman = {"mean": ht["adj_mean"], "var": ht["adj_var"]}
        if at:
            away_kalman = {"mean": at["adj_mean"], "var": at["adj_var"]}

    # Project scores
    a_proj = proj_score(gg["away"], gg["home"], False, H, a, W, away_kalman, w_var)
    h_proj = proj_score(gg["home"], gg["away"], True, H, a, W, home_kalman, w_var)
    ...
    # Clean total for over/under probability
    ...
    margin_var = (h_proj.get("variance", 0) or 0) + (a_proj.get("variance", 0) or 0)
    margin_std = math.sqrt(max(margin_var, 1))

    p_home_cover = normal_cdf(margin / margin_std)
    p_away_cover = 1 - p_home_cover

    # Total: P(over) using clean total projection
    total_var = margin_var * 1.1
    ...
    if use_bayesian:
        ...
        # Legacy threshold-based logic
        if s_diff >= W.get("sprHigh", 5):
            if margin > 0:
                s_pick = f"{gg['home']} -{abs_line}" if home_fav else f"{gg['home']} +{abs_line}"
            else:
                s_pick = f"{gg['away']} +{abs_line}" if home_fav else f"{gg['away']} -{abs_line}"
        ...
        if abs(clean_t_diff) >= W.get("ouHigh", 5):
            ou_elite_adj = W.get("ouHigh", 5) + W.get("ouEliteBump", 3)
    ...
    _margin_features = extract_margin_features(h_team, a_team, a, W.get("paceAdj", 1))
    ...
    result.update({
        ...
        "oPick": o_pick,
        "oConf": o_conf,
        "injuryNote": build_injury_note(injury_adj) if injury_adj else None,
        # Bayesian outputs
