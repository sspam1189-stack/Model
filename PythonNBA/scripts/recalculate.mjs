...
async function main() {
  ...
  console.log("  " + runs.length + " runs in history.json");
  ...
  // Compute dynamic residualVar from full history (stable at ~210-215)
  ...
  for (const run of runs) {
    ...
    // Apply daily drift (once per date change)
    ...
    for (const g of run.games || []) {
      ...
      // Use raw season stats — no home/away split blending during replay
      const gameStats = { ...gameStats_base };

      // Replay cached lineup/B2B deltas from run_daily (if present)
      if (g._adjDeltas) {
        ...
        for (const [side, teamName] of [["away", g.away], ["home", g.home]]) {
          ...
          if (delta && gameStats[teamName]) {
            ...
            for (const k of STAT_KEYS) {
              if (delta[k]) gameStats[teamName][k] = (gameStats[teamName][k] || 0) + delta[k];
            }
          }
        }
      }
      ...
      // Re-analyze with season stats + Kalman + weight variance
      const result = analyzeGame(
        { away: g.away, home: g.home, line: g.line, total: g.total },
        gameStats, gameAvgs, W, null, kalmanState, W_var, dynamicResidualVar
      );
