...
export function analyzeGame(g, H, a, W, injuryAdj = null, kalmanState = null, W_var = null, residualVar = null) {
  ...
  if (useBayesian) {
    ...
    // Spread — single tier
    // sDiff cap: 9 — wider net captures more actionable picks.
    const SDIFF_CAP = 9;
    const bestSpreadP = Math.max(pHomeCover, pAwayCover);
    const spreadSide  = pHomeCover >= pAwayCover ? "home" : "away";

    if (bestSpreadP >= probH && sDiff <= SDIFF_CAP && absLine < 12) {
      if (spreadSide === "home") {
    ...
    if (sDiff >= W.sprHigh && sDiff <= 9 && absLine < 12) {
      sPick = margin > 0
