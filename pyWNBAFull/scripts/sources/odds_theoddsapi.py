# scripts/sources/odds_theoddsapi.py  (pyWNBAFull)
# Fetch WNBA spreads + totals from The Odds API (sport key basketball_wnba).
# Requires env var: ODDS_API_KEY (falls back to embedded free-tier keys).
# Fallback source only — FanDuel is primary. Degrades gracefully (returns []).
...
def fetch_todays_odds():
    ...
    for idx, api_key in enumerate(keys):
        url = (
            f"{BASE}/sports/basketball_wnba/odds?"
