# scripts/sources/odds_fanduel.py  (pyWNBAFull)
# Fetch WNBA game spreads + totals from FanDuel's public sportsbook API.
# No API key needed — uses the same public endpoints as the FD website.
# Verified 2026-07-01: customPageId=wnba returns today's WNBA games w/ lines.
...
def fetch_fanduel_nba_odds():
    """
    Fetch today's WNBA spreads + totals from FanDuel.
    Returns list of dicts matching the format from odds_theoddsapi:
        [{"away": str, "home": str, "line": float, "total": float, "_book": "FanDuel"}]
    """
    url = f"{FD_BASE}/content-managed-page?page=CUSTOM&customPageId=wnba&_ak={FD_API_KEY}"
    try:
    ...
    print(f"  [fanduel] Fetched {len(games)} WNBA games with spreads/totals")
    return games
