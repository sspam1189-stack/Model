...
ODDS_CACHE_DIR = os.path.join(_dir, "..", "..", "data", "odds_cache", "wnba")

BASE = "https://api.the-odds-api.com/v4"

# NOTE: The Odds API *historical* endpoint requires a PAID plan. The embedded
# free-tier keys return HTTP 401 HISTORICAL_UNAVAILABLE_ON_FREE_USAGE_PLAN
# (verified 2026-07-01). With a paid ODDS_API_KEY this backfills WNBA closing
# lines; without one it raises and the backfill records MISSING_ODDS.
...
def to_model_line(home_team, away_team, spread_points, team_for_spread):
    ...
    if not is_home and not is_away:
        return None
    # Engine convention (core/model_engine.py): margin = h_s - a_s + line, with
    # home_fav = line < 0. So `line` MUST be the standard home spread —
    # NEGATIVE when the home team is favored. Emit that convention directly.
    if is_home:
