...
INJURY_CACHE_DIR = os.path.join(_dir, "..", "..", "data", "injury_cache", "wnba")
...
def fetch_injury_data(date=None, season_type="Regular Season", espn_type=2):
    ...
    WNBA MVP: the NBA injury scraper (ESPN summary parsing + NBA team-name maps)
    is not ported. WNBA projections are stat-only, which is exactly what the
    WNBA HCA/variance constants were calibrated against (raw final scores, no
    injury model). Returning an empty report makes lineup_adjust a no-op.
    Wire a real WNBA injury source here later if it proves to add edge.
    """
...
def fetch_out_for_season(game_injury_report=None):
    """
    WNBA MVP: no injury model (see fetch_injury_data). Return an empty set so
    the returning-star / out-for-season logic is a no-op.
    """
    return set()
