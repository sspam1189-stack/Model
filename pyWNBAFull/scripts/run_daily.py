#!/usr/bin/env python3
# scripts/run_daily.py  (pyWNBAFull)
# Daily WNBA full-season (spreads) picks pipeline:
...
def main(subject_label="[PY]"):
    ...
    print(f"\n== WNBA Picks (Full Season) Pipeline - {date_display} ==\n")
    ...
    # Self-contained stats cache under pyWNBAFull/data (not shared).
    _shared_cache_dir = os.path.join(_scripts_dir, "..", "data", "stats_cache", "wnba")
    ...
    # Self-contained injury cache under pyWNBAFull/data (not shared).
    _inj_cache_dir = os.path.join(_scripts, "..", "data", "injury_cache", "wnba")
    ...
    # (NBA had a hardcoded Orlando-Magic L10 blowout-scrub override here; removed
    # for WNBA — team-specific and dated.)
    ...
    recent_injury_dates = {}
    try:
        inj_cache_dir = os.path.join(os.path.dirname(__file__), "..", "data", "injury_cache", "wnba")
    ...
    # Write wnba-full.json (same JSON shape as NBA fullseason.json so grading /
    # backfill logic ports unchanged) into pyWNBAFull/data AND PythonDashboard/data.
    try:
        import shutil
        _here = os.path.dirname(os.path.abspath(__file__))
        _src = os.path.join(_here, "..", "data", "history.json")
        # 1) engine-local copy
        shutil.copy2(_src, os.path.join(_here, "..", "data", "wnba-full.json"))
        # 2) dashboard copy (best-effort — dir may not exist in all checkouts)
        _dash_dir = os.path.join(_here, "..", "..", "PythonDashboard", "data")
        try:
            os.makedirs(_dash_dir, exist_ok=True)
            shutil.copy2(_src, os.path.join(_dash_dir, "wnba-full.json"))
        except Exception as _de:
            print(f"  [dashboard] dashboard copy skipped: {_de}")
        print("[7/7] wnba-full.json written.")
    except Exception as e:
        print(f"  [dashboard] sync failed: {e}")

    print(f"\nDone: {subject_label} WNBA Picks (Full Season) {run['dateDisplay']}")
